...
//  		if(rProgram==null){
//   		   rProgram = new Rprogram();   // uncomment for raspberry pi
//   		pi4j=null;  // for debug
//   		   this.putApplicationTable("r", rProgram);
//   		}
...
//			return false;
		...
		if(Util.parseKeyWord(cmd,"R ",rest)){
			String[] names=new String[1];
			cmd=rest[0];
			boolean rtn=Util.parseName(cmd, names, rest);
			InterpreterInterface bx=this.lookUp(names[0]);
//				inqueue=new CQueue();
//				Rprogram rpro=new Rprogram();
			if(this.rProgram==null){
				this.rProgram=new Rprogram();
				this.putApplicationTable(names[0],this.rProgram);								
			}
			rProgram.parseCommand("clearOutput");
			this.inputText="";
			return true;
		}
		else
...
//			this.parseCommand("setPageName", pageName);
		...
		if(Util.parseKeyWord(cmd,"report",rest)){
			String v1=Util.skipSpace(rest[0]);
			if(!Util.parseKeyWord(v1, "=", rest)) return false;
			v1=Util.skipSpace(rest[0]);
			String rx=null;
			for(Enumeration<String> keys=applicationTable.keys();keys.hasMoreElements();){
				String key=keys.nextElement();
				if(Util.parseKeyWord(v1,key+" ",rest)){
					cmd=rest[0];
					if(!cmd.startsWith("getOutput")){
						return false;
					}
					InterpreterInterface obj=this.lookUp(key);
					rx=obj.parseCommand(cmd);
					if(rx==null) return false;
					if(rx.equals("ERROR")) return false;
					break;
				}
			}						
			if(rx.equals("ERROR")) return false;
			StringTokenizer st=new StringTokenizer(rx,"\n");
...
//					sendCommandToActivity("activity append input",line+"\n");
				String program=line.substring("R:".length());
				rtn=addProgramLine(program);
			}			
